...
- A Ow workspace with a connected provider and model that can run long running tasks like OpenAI GPT 5.4.
- Google Chrome on the machine running the session
- In `Settings > Extensions > Control Chrome`
- In `Settings > Skills > enable `chrome-mcp-bootstrap` and `screenpipe`
